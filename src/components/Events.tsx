...
export default function Events() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      ...
      (err) => console.error("Homepage events listener error:", err),
    );
    return () => unsub();
  }, []);
  ...
  const events = [
    {
      day: "15",
      month: "May",
      name: "Hussain Sagar Lake Drive",
      loc: "Hussain Sagar, Hyderabad",
      time: "8:00 AM",
      badge: "Cleanup",
      badgeClass: "bg-[#c9a84c]/15 text-gold border-[#c9a84c]/25",
    },
    {
      day: "22",
      month: "May",
      ...
      time: "9:00 AM",
      badge: "E-Waste",
      badgeClass: "bg-[#8b6f47]/20 text-sand border-[#8b6f47]/30",
    },
    {
      day: "01",
      month: "Jun",
      name: "Forest Trail Cycling — Monsoon Edition",
      loc: "Sanjay Gandhi NP, Mumbai",
      time: "6:30 AM",
      badge: "Cycling",
      badgeClass: "bg-[#2a6496]/20 text-[#87c0e8] border-[#2a6496]/30",
    },
    {
      day: "05",
      month: "Jun",
      name: "World Environment Day Mega Drive",
      loc: "Pan-India, 24 Cities",
      time: "All Day",
      badge: "Flagship",
      badgeClass: "bg-fern/20 text-sage border-fern/30",
    },
  ];

  // Combine Firebase events with static events
  const allEvents = [
    ...firebaseEvents.map((ev) => ({
      day: ev.date,
      month: ev.month,
      name: ev.title,
      loc: ev.location,
      time: ev.time,
      badge: ev.tag,
      badgeClass: "bg-fern/20 text-sage border-fern/30", // Default badge style for Firebase events
    })),
    ...events,
  ];
      ...
      <div className="mt-[60px] grid grid-cols-1 md:grid-cols-[1.2fr_1fr] gap-[60px] items-start">
        <div className="flex flex-col gap-[3px] reveal-left">
          {allEvents.map((ev, i) => (
            <div
              key={i}
              className="flex items-center gap-6 p-6 md:px-7 bg-white/5 border-l-[3px] border-transparent transition-all duration-300 cursor-pointer hover:bg-fern/10 hover:border-fern md:hover:pl-9"
            >
              <div className="text-center shrink-0 min-w-[52px]">
                <div className="font-mono text-[1.8rem] text-gold font-bold leading-none">
                  {ev.day}
                </div>
                <div className="text-[0.65rem] tracking-[2px] uppercase text-cream/40 mt-1">
                  {ev.month}
                </div>
              </div>
              <div className="flex-1">
                <div className="font-display text-[1.05rem] text-cream mb-1">
                  {ev.name}
                </div>
              ...
              <div
                className={`px-3 py-1 rounded-full text-[0.65rem] tracking-[1px] uppercase shrink-0 border ${ev.badgeClass}`}
              >
                {ev.badge}
              </div>
            </div>
        ...
        <div className="sticky top-[100px] bg-gradient-to-br from-moss/40 to-[#0d1f15]/80 border border-fern/20 rounded-[24px] overflow-hidden reveal-right">
          ...
          <div className="p-7">
            <div className="text-[0.65rem] tracking-[2px] uppercase text-sage mb-2">
              Featured Event
            </div>
            <div className="font-display text-[1.5rem] font-bold text-white mb-3">
              World Environment Day Mega Drive 2026
            </div>
            <p className="text-[0.84rem] text-cream/60 leading-[1.6] mb-6">
              Our biggest event of the year — 24 cities, 10,000 volunteers,
              100,000 saplings, and a nationwide chain of action for our rivers,
              forests, and skies.
            </p>
